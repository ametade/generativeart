...
generate_plot <- function(df, file_name, polar, filetype, color = "black", background_color = "white", dpi=300) {
  print("generate plot")
  if (polar == TRUE) {
    plot <- df %>%
  ...
  ggplot2::ggsave(plot, filename = paste0(IMG_PATH, file_name), width = 20, height = 20, unnits="in", device = filetype, dpi = 300, res = 300)
  print("image saved...")
}
